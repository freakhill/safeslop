...
function __agent_sandbox_tools_examples
    ...
    echo '  cp library/layer/container/agent-tools.env.example library/layer/container/agent-tools.env'
    echo
    echo 'Build and run the tools image:'
    echo '  docker compose --env-file library/layer/container/agent-tools.env -f library/layer/container/docker-compose.yml build agent-tools'
    echo '  docker compose --env-file library/layer/container/agent-tools.env -f library/layer/container/docker-compose.yml run --rm agent-tools'
    echo
...
function __agent_sandbox_tools_help
    ...
    echo "  ('agent-tools' service in library/layer/container/docker-compose.yml). Picks up"
    echo "  pinned versions from library/layer/container/agent-tools.env when present."
    echo ""
    echo "Usage:"
    echo "  source scripts/slop-agent-sandbox-tools.fish"
    ...
    echo "  - library/layer/container/agent-tools.env (when present) pins CLI versions for the build."
...
function __agent_sandbox_tools_tui
    ...
    while true
        ...
        if test -f library/layer/container/agent-tools.env
            gum style --faint "env file: library/layer/container/agent-tools.env (pinned versions in use)"
        else
            gum style --foreground 196 "no library/layer/container/agent-tools.env — versions will be from compose defaults"
        end
...
function __agent_sandbox_tools_compose_cmd --argument-names policy
    # We intentionally preserve support for optional library/layer/container/agent-tools.env.
    # This keeps pinned versions configurable without changing command UX.
    set -e argv[1]

    if test -f library/layer/container/agent-tools.env
        docker compose --env-file library/layer/container/agent-tools.env -f library/layer/container/docker-compose.yml $argv
    else
