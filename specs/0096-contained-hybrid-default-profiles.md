...
The projection is read-only and allowlist-only. Spec 0107 supersedes the MVP's direct source mounts: on macOS/Linux the engine walks from a pinned approved-root descriptor, accepts only relative source-path symlinks that remain inside that root, verifies exclusions/type/identity/mount identity, and copies bytes into an atomically published private per-session `0700` snapshot. Only snapshot files are mapped at `/safeslop/projected/<id>:ro`, recorded in `/safeslop/runtime/projection.json`, and copied into `/home/agent` tmpfs. The live original/resolved host pathname is never mounted or reopened after validation. Spec 0108 narrows optional glob membership only: physical regular matches are copied, while terminal links/directories/special files are never followed or opened and produce one aggregate `skipped-nonregular` status. Direct sources, required globs, recursive trees, excluded targets, loops, mount crossings, concurrent changes, and unsupported platforms remain fail-closed with no pathname fallback. Workspace remains the only read-write host mount. See `specs/research/2026-07-12-safe-home-projection-flo.md`, `specs/research/2026-07-16-symlinked-projection-flo.md`, and `specs/research/2026-07-16-optional-projection-globs-flo.md`.
