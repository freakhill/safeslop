...
Ten hermetic journey tests then passed: first-run profile discovery with no credential rows, account confirmation/context, truthful success next steps, existing mixed-scope prefill and replacement warning, profile-only forge clear, value-free failed account/scope draft retry, unlink failure recovery, guidance, and universal key dispatch. The raw/Evil/Doom-Evil UI matrix resolves every displayed list and inspect action, including state-aware `g`/`gr`. Mixed GitHub/Forgejo inspection now reports each repository's own read/write access, and account probe classes carry value-free explanations.

A cross-family post-fix evaluator first scored the artifact **75/100** after finding a hidden inspect-buffer Evil mismatch despite the list journeys passing. After making inspect a real derived mode, binding/testing `e`/`gr`, and correcting stale lowercase recovery text, re-evaluation reached **92.25/100** with no blockers. Its remaining concrete gaps—unlink-failure return guidance, no-policy link next step, account-link retry defaults, staging consequence warning, and probe help—were then fixed and regression-tested before full gates.
