...
- [x] Canonicalize workspace authority and render typed Compose mounts
...
- [x] Pin and harden proxy and npm build inputs
...
- [x] Make live egress updates direction-aware and acknowledged
...
- [x] Synchronize operator contracts and spec evidence
  FILE:     `README.md`, `CONTRIBUTING.md`, `emacs/README.md`, `skills/README.md`, `skills/agent-sandbox-ops/SKILL.md`, `skills/agent-key-lifecycle/SKILL.md`, `library/README.md`, `specs/0115-code-quality-hardening.md`
  CHANGE:   Documented policy-relative canonical workspaces, hostile-path rejection versus valid quoting, unique invocation ownership, typed value-free corruption/stale/commit uncertainty, proxy-replacement grant/revoke generation/hash ACKs, uncertainty teardown and blocked retry semantics, terminal teardown retries, legacy session layout/backend/bootstrap cleanup, digest-pinned hardened Squid, per-package npm package-lock/SRI and closed script/binary policy, credential-free selected build contexts, honest non-hermetic/non-bit-reproducible networked builds, current make/CI commands, and unchanged v1/value-free/public boundaries. Removed active VM operator instructions and obsolete image-surface wording while leaving historical specs as historical evidence only.
