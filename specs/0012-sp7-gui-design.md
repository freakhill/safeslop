...
- **Preferred terminal**, from a supported list: `Terminal.app`, `Ghostty`, `generic` (`iTerm2`, `kitty`/`wezterm` later). **Terminal.app** (and `generic`/unknown — the always-present fallback) opens via AppleScript `do script` in a new window's login shell. **Ghostty** opens via `open -na Ghostty --args -e <shell> -lc <command>` (its `-e` takes a program, so the command is wrapped in the user's preferred shell). The session command is built shell-quoted (`internal/engine/launch`).
...
4. **Terminal adapters scope for SP7a:** `Terminal.app` (AppleScript `do script`) + `Ghostty` (`-e <shell> -lc`) + `generic`→Terminal fallback in v1; `iTerm2` native tagging + `kitty`/`wezterm` follow-on. **Recommendation:** as written.
