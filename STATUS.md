...
* **Portal** (Sessions; `M-x safeslop`, alias `safeslop-portal`, `C-c s P`): a `tabulated-list` session dashboard — id, agent, environment, network, colour-coded status, PID, age, workspace — with in-place actions (open/attach, reattach, status, stop, new, refresh). The **Env** cell is now coloured by isolation tier (host red, container green, mirroring `policy.EnvTier`) with the honest one-line caveat on hover and a tier-ramp legend; colour only reinforces the always-present label. Creating a session reveals it in the portal and offers to open it right away. Opens full-window, auto-refreshing in place.
