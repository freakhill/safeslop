...
- **Network**: route the agent through a proxy (`library/squid.conf`)
...
- Force outbound traffic through a proxy with a deny-by-default
  allowlist. Start from `library/allowlist.domains` and
  `library/squid.conf`. Add a host-level firewall ([LuLu](https://objective-see.org/products/lulu.html)
...
- Pin all installed tools to exact versions (`library/agent-tools.env`,
...
**Claude Code:** start from `library/claude-code.settings.json`.
...
**OpenCode:** load `library/opencode.restrictive.json` via
`OPENCODE_CONFIG`, then run OpenCode inside the `agent` container from
`library/docker-compose.yml`. App-level URL allowlist is not enough on
...
**OpenClaw:** start from `library/openclaw.restrictive.md`. Because
...
from `library/docker-compose.yml`; treat `SOUL.md` and any persona
...
**ZeroClaw:** start from `library/zeroclaw.restrictive.md`. ZeroClaw
...
Reference config: `library/opencode.restrictive.json`
...
Reference policy: `library/openclaw.restrictive.md`.
...
Reference policy: `library/zeroclaw.restrictive.md`.

### Claude Code sandbox reference

Use `/sandbox` in Claude Code and keep filesystem/network policy strict. Example file: `library/claude-code.settings.json`.
...
- Keep pinned framework versions in `library/agent-tools.env`
...
```fish
docker compose -f library/docker-compose.yml build agent
docker compose -f library/docker-compose.yml up -d proxy
```

3. Run agent container through proxy:

```fish
docker compose -f library/docker-compose.yml run --rm agent
```

4. Verify blocking:

```fish
docker compose -f library/docker-compose.yml run --rm agent sh -lc 'curl -I https://example.com'
```
...
```fish
cp library/agent-tools.env.example library/agent-tools.env
```

2. Edit `library/agent-tools.env` and enable only the stacks you need
3. Keep versions pinned; avoid `latest` in automation
4. Build and run the tools image:

```fish
docker compose --env-file library/agent-tools.env -f library/docker-compose.yml build agent-tools
docker compose --env-file library/agent-tools.env -f library/docker-compose.yml run --rm agent-tools
```
...
`library/isolation/presets/`, mirrored as fixtures.
...
```fish
set -x OPENCODE_CONFIG (pwd)/library/opencode.restrictive.json
```

2. Run OpenCode inside the `agent` container from `library/docker-compose.yml`
...
1. Read `library/openclaw.restrictive.md` and apply its policy to
   your local OpenClaw config.
2. Run OpenClaw inside the `agent` container from
   `library/docker-compose.yml`. Do not run it on the host.
3. Override the workspace location so it does not default to the
   user's home directory:

```fish
docker compose -f library/docker-compose.yml run --rm \
...
5. Add the channel's API host to `library/allowlist.domains` and
...
1. Read `library/zeroclaw.restrictive.md` and apply its policy to
...
1. Enable `/sandbox`
2. Apply settings similar to `library/claude-code.settings.json`
...
Reference: `library/tart-brew-sandbox.md`

### How to strengthen network limiting

1. Keep default script mode as `strict-egress`
2. Maintain outbound allowlist in `library/allowlist.domains`
...
Reference template: `library/forgejo-instances.example.json`
...
Reference state format: `library/radicle-access-policy.example.json`
...
```fish
set -x OPENCODE_CONFIG /workspace/library/opencode.restrictive.json
...
- `library/tart-brew-sandbox.md`: VM template assumptions for `slop-brew-vm`
...
- `library/forgejo-instances.example.json`: sample multi-instance Forgejo profile file
- `library/radicle-access-policy.example.json`: sample Radicle identity/binding state format
...
- `library/docker-compose.yml`: reusable agent + proxy stack
- `library/allowlist.domains`: central outbound domain allowlist used by proxy
- `library/Dockerfile.agent`: custom agent image with fish + uv + Python + Node
- `library/Dockerfile.agent.tools`: optional layer with preinstalled agent stacks
- `library/agent-tools.env.example`: pinned package/version template
- `library/squid.conf`: URL allowlist rules
- `library/opencode.restrictive.json`: restrictive OpenCode permissions
- `library/claude-code.settings.json`: restrictive Claude Code sandbox settings
- `library/openclaw.restrictive.md`: restrictive OpenClaw policy (channels, workspace, SOUL.md handling)
- `library/zeroclaw.restrictive.md`: restrictive ZeroClaw policy (workspace boundary, supervised autonomy, tool receipts)
...
5. Keep allowlist in `library/allowlist.domains` and verify denials regularly
...
cp library/agent-tools.env.example library/agent-tools.env
docker compose --env-file library/agent-tools.env -f library/docker-compose.yml build agent-tools
docker compose --env-file library/agent-tools.env -f library/docker-compose.yml run --rm agent-tools sh -lc 'python3 --version && node --version && uv --version'
docker compose --env-file library/agent-tools.env -f library/docker-compose.yml run --rm agent-tools sh -lc 'python3 -m pip show crewai pydantic-ai pydantic-ai-harness ag2'
```
