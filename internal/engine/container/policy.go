...
func RenderSessionGrants(grants []SessionGrant) string {
	...
	if len(grants) == 0 {
		b.WriteString("# generated by safeslop; no session egress grants (final deny-all remains)\n")
		return b.String()
	}
	for i, g := range grants {
		escaped := strings.ReplaceAll(g.Host, ".", `\.`)
		fmt.Fprintf(&b, "acl grant_%d_host dstdom_regex -n ^%s$\n", i, escaped)
		fmt.Fprintf(&b, "acl grant_%d_port port %d\n", i, g.Port)
		fmt.Fprintf(&b, "http_access allow grant_%d_host grant_%d_port\n", i, i)
	}
	return b.String()
}
