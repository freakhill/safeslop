...
func TestStageGitHubPATMultiRepo(t *testing.T) {
	...
	env, err := StageSSH(context.Background(), &policy.Credentials{Ssh: &policy.SshCreds{
		Mode: "pat",
...
func TestStagePATRequiresTokenAndRepos(t *testing.T) {
	if _, err := StageSSH(context.Background(), &policy.Credentials{Ssh: &policy.SshCreds{Mode: "pat", Repos: []policy.RepoCred{{Repo: "acme/web"}}}}, t.TempDir()); err == nil {
