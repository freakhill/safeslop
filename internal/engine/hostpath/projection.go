...
func SnapshotProjection(home, stageDir string, proj policy.Projection) (manifest ProjectionManifest, err error) {
	...
	r.homeProof, err = openProofRoot(home)
	if err != nil {
		return ProjectionManifest{}, newProjectionError(ProjectionSafetyUnsupported, policy.ProjectionItem{}, home, true)
	}
	defer r.homeProof.close()
	r.homeRoot, r.homeInfo, r.homeMount = r.homeProof.root, r.homeProof.before, r.homeProof.mountID
	defer r.closeProofs()
	...
	if needsXDG {
		r.xdgProof, err = openProofRoot(xdg)
		if err != nil {
			return ProjectionManifest{}, newProjectionError(ProjectionSafetyUnsupported, policy.ProjectionItem{}, xdg, true)
		}
		defer r.xdgProof.close()
		r.xdgRoot, r.xdgInfo, r.xdgMount = r.xdgProof.root, r.xdgProof.before, r.xdgProof.mountID
	}
...
func (r *projectionResolver) openPinned(root *os.Root, rootMount uint64, rel string) (*pinnedNode, error) {
	proof := r.proofFor(root, rootMount)
	node, err := openPinnedPath(proof, rel, func(canonical string) error {
		if code := projectionLawCode(canonical, r.home, r.xdg, r.excluded); code != "" {
			return r.fail(code)
		}
		return nil
	}, func() {
...
func (r *projectionResolver) proofFor(root *os.Root, mountID uint64) *proofRoot {
	if r.homeProof != nil && root == r.homeProof.root {
		return r.homeProof
	}
	if r.xdgProof != nil && root == r.xdgProof.root {
		return r.xdgProof
	}
	return &proofRoot{path: root.Name(), root: root, mountID: mountID}
}

func (r *projectionResolver) mapProofError(err error) error {
	...
	case proofUnavailable:
		return unavailableError{absent: proofErr.absent}
	case proofOutsideRoot:
		return r.fail(ProjectionTargetOutsideRoot)
	case proofSymlinkLoop:
		return r.fail(ProjectionSymlinkLoop)
	case proofChanged:
		return r.fail(ProjectionSnapshotChanged)
	case proofUnsupported:
		return r.fail(ProjectionSafetyUnsupported)
	case proofUnsafe:
		return r.fail(ProjectionUnsafeDescendant)
	case proofSourceType:
		return r.fail(ProjectionSourceType)
	default:
		return r.fail(ProjectionSafetyUnsupported)
	}
}
...
// ProjectionBoundaryError returns the value-free typed failure used when a
// caller detects that an engine-owned snapshot path escaped its custody root.
