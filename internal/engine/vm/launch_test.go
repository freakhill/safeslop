...
func TestLaunchRejectsWhenUnavailable(t *testing.T) {
	t.Setenv("PATH", "")
	_, err := Launch(context.Background(), exec.LaunchSpec{Argv: []string{"zsh"}}, "allow", nil, t.TempDir(), "p", "")
	if err == nil {
...
func TestLaunchDenyNeedsProxyURL(t *testing.T) {
	t.Setenv("PATH", "")
	t.Setenv("SAFESLOP_VM_PROXY_URL", "")
	_, err := Launch(context.Background(), exec.LaunchSpec{Argv: []string{"zsh"}}, "deny", nil, t.TempDir(), "p", "")
	if err == nil {
